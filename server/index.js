...
app.post('/api/reset-password', async (req, res) => {
  ...
  try {
    ...
    return res.status(422).send('Oops, something went wrong');
  }
})
...
app.get('/api/activate/user/:hash', async (req, res) => {
  ...
  try {
    ...
    res.status(422).send('Error in activation');
  }
})
...
app.post('/api/register', async (req, res) => {
  ...
  try {
    ...
    await newUser.save();
    res.json({message: 'You are registered. Check your email address'});
  } catch(e) {
